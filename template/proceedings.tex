...
Multiphonics is a set of techniques that make single pitch sound sources (e.g. single strings and wind instruments) produce sounds with more than one perceivable pitch. On strings, multiphonics are performed by lightly touching the string at positions other than those producing harmonics.
...
\begin{figure}[h]
	...
	\caption{A digital waveguide model with a scattering junction. \textit{Bridge-finger} loop (red), \textit{finger-nut} loop (green) and \textit{bridge-nut} loop (blue)}
