...
#import "@preview/fletcher:0.5.8" as fletcher: node as fletchernode
...
#waveguide(
  (
    ...
    dot((1, 0), name: <dot-1>),
    mul((2, 0), [$rho_2$], dir: right, side: left),
    add((3, 0), name: <add-1>),
  ...
  (
    ...
    dot((3, 2), name: <dot-2>),
    mul((2, 2), [$rho_2$], dir: left, side: left),
    add((1, 2), name: <add-2>),
  ...
  mul((3, 1), [$rho_1$], name: <mul-2-1>, dir: top, side: right),
  mul((1, 1), [$rho_1$], name: <mul-1-2>, dir: bottom, side: left),
  // down
  arrow(<dot-1>, <mul-1-2>),
  arrow(<mul-1-2>, <add-2>),
